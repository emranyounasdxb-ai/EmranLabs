...
export function DesktopAppIcon({
  ...
  return (
    <motion.button
      ...
      whileHover={app.enabled ? { y: -3, scale: 1.025 } : undefined}
      whileTap={app.enabled ? { scale: 0.965 } : undefined}
      transition={motionTransitions.fast}
      className="group relative flex min-h-12 min-w-12 flex-col items-center justify-center gap-2 rounded-2xl border border-[var(--glass-border)] bg-white/[0.055] p-3 text-[var(--text-primary)] shadow-[var(--shadow-panel)] backdrop-blur-xl transition-[background-color,border-color,box-shadow] duration-[var(--duration-standard)] ease-[var(--motion-ease)] hover:border-[rgba(23,227,192,0.42)] hover:bg-white/[0.085] disabled:cursor-not-allowed disabled:opacity-45"
    >
      ...
      {(active || minimized) && (
        <motion.span
          ...
          className="absolute bottom-1 h-1 w-5 rounded-full bg-[var(--color-signal)] opacity-80"
        />
      )}
      {!app.enabled && (
